...
#load token images
#boot_done, smartphone_done, ship_done, hatstand_done, cat_done and iron_done
...
#load popup images
popup = pygame.image.load('resources/popup_demo.png').convert_alpha()

#create popup instance
popup_demo = PopUp.PopUp(450, 300, popup, 1)
ok_demo_button = Button.Button(490, 350, ok, 1)
...
def start_screen():
    ...
    while True:
        ...
        if popup_show == True:
            popup_demo.draw(screen, 'An example popup')
            ok_demo_button.draw(screen)
            if ok_demo_button.click():
                popup_show = False
    ...
    while True:
        ...
        if len(list_banker) > 1 and dice_rolled == False:
            ...
            if bigdice_button.click():
                ...
                #banker = bk.bank_creations(our_players[index].player, 50000, our_players[index].probability)
...
def timer_mode():
    ...
    while len(our_players) != 1:
        ...
        # check_mortgage = our_players[count].check_mortgage()U
        ...
        if dice_board_button.click() and press == False:
            ...
            roll = (3,3)
            if roll[0] == roll[1]:
                #text = font.render("Rolled a double", True, (0, 0, 0), (255, 255, 255))
            ...
            # board_card = Token.Token(530, 267, bd.board_images(our_players[count].position), 0.8)
            # board_card.draw(screen, 530, 267)
            # our_players[count].check_double(our_players, roll[0], roll[1])
...
def start_game():
    ...
    while len(our_players) != 1:
        ...
        #check_mortgage = our_players[count].check_mortgage()
    ...
    while auction == False:
        ...
        if len(player_bet) == 0 and len(players) == 0:
            ...
            if skip_button.click():
                print(players[count].player, "has elinimated himself")
                del player_bet[players[count].player]
                del players[count]
                count -= 1
                if count >= len(players) or count < 0:
            ...
            if gb[player.position].card_owner == player.player:
                ...
                if buy_button.click():
                    ...
                    if player.money <= gb[player.position].card_cost and len(player.cards) >= 1:
                        ...
                        #check_sellable(player, gb[player.position].card_cost, pay_player=False)
                ...
                if cancel_button.click():
                    ...
                    #our_players[count].auction(gb[our_players[count].position], our_players)
            else:
                card_show = False
    ...
    while card:
        ...
        if player.probability > 0:
            ...
            if (house + hotel == 0):
                ...
                print("Since ",player.player , " owns ", house, " houses and ", hotel, "hotel", " the duducted moeny is ", house*payment[0] + hotel*payment[1])
        ...
        if pay_button.click():
            ...
            if (house + hotel == 0):
                ...
                if player.money < total_payment:
                    ...
                    #check_sellable(player, total_payment, pay_player=False)
                else:
                    gb[20].rent += player.deduct_money(total_payment)
                card = False
...
def jail(player):
    ...
    while jail:
        ...
        if exit_jail_button.click() and player.jail_count == 5:
            ...
            jail = False
            player.jail_count = 0
        if wait_till_release_button.hover() and player.jail_count <4:
            wait_till_release_button_hover.draw(screen)
        else:
            wait_till_release_button.draw(screen)
        if wait_till_release_button.click() and player.jail_count <4:
            click = True
            xdskip = True
...
def sell_card(player, amount, pay_player):
    ...
    while (flag):
        ...
        if sell_page_button.click() and click:
            ...
            # if int(select) <= len(self.cards) - 1:
            # if card_color == self.cards[int(select)].card_color:
            # print("You cannot sell the same card")
            if len(player.cards) == 0:
                click = False
            ...
            # print(player.cards[index].card_cost)
...
def auction_menu(players,card,idx):
    bet = card.card_cost
    count = 0
    player_bet = {i.player: 0 for i in players if i.circuit == True}
    print("In")
    auction = False
    while auction == False:
        #print(players[count].player)
        ...
        if len(player_bet) == 0 and len(players) == 0:
            ...
            if count >= len(players):
                count = 0
            #print("Current player is", players[count].player)
...
def jail(player):
    ...
    while jail:
        screen.blit(board, (0, 0))
        for special_card in player.stored_jail_cards:
            if special_card.card_name == "Jail_Pot":
                pot = True
            if special_card.card_name == "Jail_Knocks":
                OK = True
        if skip_button.hover() and player.jail_count <=3:
            skip_button_hover.draw(screen)
        else:
            skip_button.draw(screen)
        if skip_button.click() and player.jail_count <=3:
            player.turns_spent_jail()
            jail = False

        if buy_button.hover() and player.jail_count == 0:
            buy_button_hover.draw(screen)
        else:
            buy_button.draw(screen)
        if buy_button.click() and player.jail_count == 0:
            player.buy_out_of_jail()
            jail = False

        if yes_button.hover() and pot:
            print('as')
            yes_button_hover.draw(screen)
        else:
            yes_button.draw(screen)
        if yes_button.click() and pot:
            player.use_jail_pass_name(card_name="Jail_Pot")
            jail = False

        if no_button.hover() and (OK and player.jail_count == 0):
            no_button_hover.draw(screen)
        else:
            no_button.draw(screen)
        if no_button.click() and (OK and player.jail_count == 0):
            player.use_jail_pass_name(card_name="Jail_Knocks")
            jail = False

        if back_button.hover() and player.jail_count == 3:
            back_button_hover.draw(screen)
        else:
            back_button.draw(screen)
        if back_button.click() and player.jail_count == 3:
            print(player.player, " is just visiting in jail")
            player.in_jail = False
            player.jail_count = 0
            jail = False
