            ...
            if int(self.card_cost) > int(token.money):
                ...
                if token.probability > 0:
                    ...
                    if self.card_color == "Railroad":
                        ...
                        token.utlity += 1
                    token.cards.append(self)
                    print(self.card_name, "This cards current owners is", self.card_owner, "currently spent:", self.card_cost, "current\n balance:",token.money)
    '''
    ...
    def buy_houses(self, token, banker):
        ...
        if len(card_list) > 0:
            ...
            if select not in check_mortgage.keys():
                        ...
                        if specfic_card.houses == 5:
                            ...
                            if token.money >= specfic_card.house_cost:
                                ...
                                if buy == "y":
                                    ...
                                    bk.bank_creations.add_bank_money(bank, token.deduct_money(specfic_card.house_cost), token)
                ...
                elif (select == "Green" or select == "Blue" or select == "Yellow" or select == "Purple" or select == "Orange" or select == "Red") and (select in card_list):
                    print(mydict[select])
                    for specfic_card in mydict[select]:
                        index_max = max(token.cards.index(mydict[select][0]),token.cards.index(mydict[select][1]),token.cards.index(mydict[select][2]))
                        if specfic_card.houses == 5:
                            print("You have reached max amount of properties")
                        elif specfic_card.houses <= token.cards[index_max].houses:
                            print("To buy a house in,", specfic_card.card_name, " will cost", specfic_card.house_cost)
                            if token.money >= specfic_card.house_cost:
                                if token.probability > 0:
                                    ...
                                    buy = input("Press y to buy a house or n to skip")
                                if buy == "y":
                                    bk.bank_creations.add_bank_money(banker,token.deduct_money(specfic_card.house_cost), token)
                                    specfic_card.houses += 1
                                    if flag:
                                        index_temp = token.cards.index(temp)
                                        token.cards[index_temp] = token.cards[index_max]
        ...
        if len(mortgage.keys()) > 0:
            for color, card in mortgage.items():
                ...
                for card_attributes in card:
                    print("card number", count, "card color", color, " has a mortgaged card ", card_attributes.card_name)
                    count += 1
            ...
            if select in mortgage.keys():
                ...
                if int(number) > len(mortgage[select]) - 1:
                    ...
                    payment = (mortgage[select][int(number)].card_cost*0.5) + (mortgage[select][int(number)].card_cost*0.5) * 0.1
                    print("The selected card to unmortgage is ", mortgage[select][int(number)].card_name, " you will need to pay", payment)
